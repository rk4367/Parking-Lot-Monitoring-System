...
# Create frame queues for more efficient streaming
frame_queues = {
    '1': Queue(maxsize=5),  # Limit queue size to prevent memory issues
    '2': Queue(maxsize=5)
}
...
def update_parking_data():
    ...
    # Get FPS from videos for proper timing
    video_fps = [30, 30]  # Default values
    ...
    while True:
        for i, (cap, positions) in enumerate(zip(caps, positions_list)):
            ...
            # Calculate time-based frame skipping to maintain original video speed
            current_time = time.time()
            elapsed = current_time - last_update_time[i]
            ...
            if frames_to_process > 0:
                ...
                frame_counters[i] += 1
                if frame_counters[i] % 10 == 0:  # Only update history every 10 frames
                ...
                # Compress frame with high performance settings
                ret, buffer = cv2.imencode('.jpg', processed_frame, [cv2.IMWRITE_JPEG_QUALITY, 70])
                if ret:
                    # Update the queue (discard old frames if queue is full)
...
def generate_frames(lot_id):
    # Create a blank frame for when no data is available
...
if __name__ == '__main__':
    ...
    # Run the Flask app - disable reloader in debug mode to prevent duplicate threads
